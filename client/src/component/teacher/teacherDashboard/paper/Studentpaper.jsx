...
import { FiEdit, FiEye, FiSearch, FiFilter, FiX, FiPlus, FiCheck, FiXCircle, FiChevronDown, FiChevronUp } from "react-icons/fi";
...
const Studentpaper = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/teacher/all-submissions`, {
        headers: {
  ...
  const filteredSubmissions = submissions.filter(submission => {
    const matchesSearch = 
      ...
      submission.contentItem.title.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesFilter = 
      filter === "all" || 
      (filter === "ungraded" && submission.gradingStatus !== "manually-graded") ||
  ...
  const calculateResults = (score, maxScore, submission) => {
    const calculatedPercentage = maxScore > 0 ? Math.round((score / maxScore) * 100) : 0;
    const passingScore = submission?.contentItem?.passingScore || 70;
  ...
  const handleGradeChange = (question, field, value) => {
    const updatedGradingData = {
      ...
      [question]: {
        ...
        [field]: value,
        isCorrect: field === 'marks' ? (Number(value) >= (gradingData[question].maxMarks * 0.5)) : gradingData[question].isCorrect
      }
    };
  ...
  const autoGradeMCQ = (question) => {
    ...
    if (type === 'mcq-single') {
      ...
      if (Array.isArray(studentAnswer) && Array.isArray(correctAnswer)) {
        isCorrect = studentAnswer.length === correctAnswer.length &&
                        ...
                        {filteredSubmissions.length > 0 ? (
                          filteredSubmissions.map((submission, index) => (
                            <tr key={index} className="hover:bg-gray-50 transition-colors">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">{submission.contentItem.title}</div>
                                <div className="text-sm text-gray-500 capitalize">{submission.contentItem.type}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">
                                  {submission.score !== undefined ? `${submission.score}/${submission.maxScore}` : 'Not graded'}
                                </div>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                  ${submission.gradingStatus === 'manually-graded' ? 'bg-green-100 text-green-800' :
                                    submission.gradingStatus === 'partially-graded' ? 'bg-yellow-100 text-yellow-800' :
                                      'bg-blue-100 text-blue-800'}`}>
                                  {submission.gradingStatus === 'manually-graded' ? 'Graded' :
                                    submission.gradingStatus === 'partially-graded' ? 'Partially Graded' : 'Not Graded'}
                                </span>
                ...
                <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        <p className="text-lg font-medium text-gray-800">{selectedSubmission.courseTitle}</p>
                      </div>
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <h3 className="text-sm font-medium text-gray-500 mb-1">Assignment Type</h3>
                        <p className="text-lg font-medium text-gray-800 capitalize">{selectedSubmission.contentItem.type}</p>
                      </div>
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <h3 className="text-sm font-medium text-gray-500 mb-1">Submission Date</h3>
                        <p className="text-lg font-medium text-gray-800">
                          {new Date(selectedSubmission.lastAccessed).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mb-6 p-4 rounded-lg border border-gray-200 bg-blue-50">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                        <div className="mb-3 md:mb-0">
                          <h3 className="text-lg font-semibold text-gray-800">Grading Summary</h3>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setTotalScore(0);
                              calculateResults(0, maxTotalScore, selectedSubmission);
                            }}
                          >
                      ...
                      {selectedSubmission.answers.map((answer, index) => (
                        <div key={index} className="border rounded-lg overflow-hidden">
                          ...
                          <div 
                            ...
                            onClick={() => toggleQuestionExpansion(answer.question)}
                          >
                            <div className="flex items-center">
                            ...
                            <div className="flex items-center">
                              <span className={`px-2 py-1 text-xs font-medium rounded mr-3 ${
                                ...
                                {gradingData[answer.question]?.isCorrect ? 'Correct' : 'Incorrect'}
                              </span>
                              <span className="text-sm text-gray-500 mr-3">
                                {gradingData[answer.question]?.marks || 0}/{answer.maxMarks} pts
                              </span>
                          ...
                          {expandedQuestions[answer.question] && (
                            <div className="p-4 bg-white">
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center">
                                    <input
                                      ...
                                      max={answer.maxMarks}
                                      step="0.5"
                                      value={gradingData[answer.question]?.marks || 0}
                                      onChange={(e) => {
                                        const value = Math.min(
                                          Math.max(Number(e.target.value), 0),
                                          answer.maxMarks
                                        );
                                        handleGradeChange(answer.question, 'marks', value);
                                      }}
                                      className="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                    />
                                    <span className="ml-2 text-sm text-gray-500">/ {answer.maxMarks}</span>
                                  </div>
                                </div>
                                <div>
                                  <label htmlFor={`feedback-${index}`} className="block text-sm font-medium text-gray-700 mb-1">
                                    Feedback
                                  </label>
                                  <textarea
                                    id={`feedback-${index}`}
                                    rows={2}
                                    value={gradingData[answer.question]?.feedback || ""}
                                    onChange={(e) => handleGradeChange(answer.question, 'feedback', e.target.value)}
                                    className="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                              ...
                              <div className="flex justify-end mt-4 space-x-2">
                                <button
                                  onClick={() => toggleCorrectStatus(answer.question)}
                                ...
                                >
                                  {gradingData[answer.question]?.isCorrect ? (
                                    <>
                                      <FiCheck className="mr-1" /> Mark Incorrect
                                    </>
                                  ) : (
                                    <>
                                      <FiXCircle className="mr-1" /> Mark Correct
                                    </>
                                  )}
                                </button>
                                {(answer.questionType === 'mcq-single' || answer.questionType === 'mcq-multiple') && (
                                  <button
                                    onClick={() => autoGradeMCQ(answer.question)}
                  ...
                  <div className="bg-gray-50 px-6 py-4 border-t flex justify-between items-center">
                    <div className="text-sm text-gray-600">
                      Grading as: <span className="font-medium">{teacherData?.full_name || "Teacher"}</span>
                    </div>
