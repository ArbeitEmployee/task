...
const Liveclass = () => {
  ...
  const fetchLiveClassAttendance = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/teacher/live-class-attendance`, {
        headers: {
  ...
  const completeLiveClass = async (courseId, contentId) => {
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to complete live class");
    }
  };

  // Update individual student attendance
  const updateStudentAttendance = async (courseId, contentId, studentId, completed) => {
    try {
      const response = await axios.post(
      ...
      if(response.data.success) {
        toast.success(`Attendance ${completed ? 'marked' : 'unmarked'} for student`);
        fetchLiveClassAttendance();
      } else {
        toast.error(response.data.message);
      }
          ...
          >
            <div className="w-full mx-auto">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <h1 className="text-2xl font-bold text-gray-800">Live Class Attendance</h1>
                          ...
                          <tr key={index} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              ...
                              <div className="text-sm text-gray-500">
                                {new Date(submission.contentItem.schedule).toLocaleString()}
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        <h2 className="text-xl font-bold">
                          Attendance Details: {selectedSubmission.contentItem.title}
                        </h2>
                  ...
                  <div className="flex-1 overflow-y-auto p-6">
                    <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <h3 className="text-sm font-medium text-gray-500 mb-1">Class Schedule</h3>
                        <p className="text-lg font-medium text-gray-800">
                          {new Date(selectedSubmission.contentItem.schedule).toLocaleString()}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mb-6 p-4 rounded-lg border border-gray-200 bg-blue-50">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                        <div className="mb-3 md:mb-0">
                          <h3 className="text-lg font-semibold text-gray-800">Attendance Status</h3>
                          <div className="flex items-center mt-1">
                            <span className={`text-xl font-bold ${
                              selectedSubmission.completed ? 'text-green-600' : 'text-yellow-600'
                            }`}>
                              {selectedSubmission.completed ? 'Completed' : 'Pending'}
                            </span>
                            {selectedSubmission.lastAccessed && (
                              <span className="ml-3 text-sm text-gray-600">
                                Last accessed: {new Date(selectedSubmission.lastAccessed).toLocaleString()}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="border rounded-lg overflow-hidden">
                        ...
                        <div className="p-4 bg-white">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-sm text-gray-800">
                                {selectedSubmission.contentItem.duration || 60} minutes
                              </p>
                            </div>
                            <div>
                              <h4 className="text-sm font-medium text-gray-700 mb-1">Time Spent</h4>
                              <p className="text-sm text-gray-800">
                          ...
                          <div className="mt-4">
                            ...
                            <p className="text-sm text-gray-800">
                              {selectedSubmission.contentItem.description || 'No description provided'}
                            </p>
                          </div>
                        </div>
                      </div>
